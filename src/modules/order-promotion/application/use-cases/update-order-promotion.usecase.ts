...
export class UpdateOrderPromotionUseCase {
  ...
  async execute(dto: UpdateOrderPromotionDto & { id: string }): Promise<Result<OrderPromotionDto, HttpError>> {
    const orderPromotion = await this.orderPromotionRepository.findById(dto.id);
