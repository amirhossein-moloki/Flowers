...
export class ProofOfDeliveryController {
  ...
  async create(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    ...
    if (result.success) {
      return res.status(201).json(ProofOfDeliveryPresenter.toJSON(result.value));
    }
    next(result.error);
  }

  async findById(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    const result = await this.findProofOfDeliveryByIdUseCase.execute(req.params.id);
    if (result.success) {
      return res.status(200).json(ProofOfDeliveryPresenter.toJSON(result.value));
    }
    next(result.error);
  }
  ...
  async delete(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    const result = await this.deleteProofOfDeliveryUseCase.execute(req.params.id);
    if (result.success) {
