...
export class CreateCustomerAddressUseCase {
  ...
  async execute(dto: CreateCustomerAddressDto): Promise<Result<CustomerAddressDto, HttpError>> {
    ...
    if (!customerAddressResult.success) {
      return failure(HttpError.badRequest(customerAddressResult.error.message));
    }
    ...
    const newCustomerAddress = await this.customerAddressRepository.findById(customerAddress.id);
