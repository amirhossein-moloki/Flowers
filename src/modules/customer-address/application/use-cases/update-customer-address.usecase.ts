...
export class UpdateCustomerAddressUseCase {
  ...
  async execute(id: string, dto: UpdateCustomerAddressDto): Promise<Result<CustomerAddressDto, HttpError>> {
    ...
    if(!updatedCustomerAddressResult.success){
        return failure(HttpError.internalServerError(updatedCustomerAddressResult.error.message));
    }
    ...
    const customerAddressDto = CustomerAddressMapper.toDto(updatedCustomerAddress);
